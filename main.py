...
def theme_extensions(device: str) -> tuple[str, ...]:
    return {
        "ios": (".tgios-theme",),
        "android": (".attheme",),
        "windows": (".tdesktop-theme", ".tgdesktop-theme"),  # Legacy suffix kept for compatibility.
    }.get(device, tuple())
...
async def process_random_theme_device(call: CallbackQuery, state: FSMContext):
    ...
    if theme_path and os.path.exists(theme_path):
        ...
        await bot.send_document(call.from_user.id, document=FSInputFile(theme_path, filename=output_name),
...
async def process_bg_device(call: CallbackQuery, state: FSMContext):
    ...
    colors_file = None
    if device == "windows":
        # Telegram Desktop expects a zip .tdesktop-theme with colors + wallpaper files.
...
async def install_theme(call: CallbackQuery):
    ...
    if theme_file and os.path.exists(theme_file):
        ...
        await bot.send_document(call.from_user.id, document=FSInputFile(theme_file, filename=output_name),
